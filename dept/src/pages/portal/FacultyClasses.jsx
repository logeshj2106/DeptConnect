    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <div className="bg-white rounded-lg p-6 shadow-md border-l-4 border-l-teal-600">
                ...
                <p className="text-gray-500 text-sm mt-1">Select a class and mark student attendance</p>
            </div>
            ...
            {!selectedClass ? (
                <div className="bg-white rounded-lg shadow overflow-hidden border border-gray-100">
                    ...
                    <div className="divide-y divide-gray-100">
                        {classes.map((cls) => (
                            <div key={cls.id} className="flex items-center gap-4 p-4 hover:bg-gray-50 transition-colors cursor-pointer"
                                onClick={() => { setSelectedClass(cls); setAttendance({}) }}>
                                <div className="flex-1">
                                    <p className="text-gray-900 font-medium">{cls.name}</p>
                                    <p className="text-gray-500 text-sm mt-0.5">Section {cls.section} • {cls.time} • {cls.room} • {cls.students} Students</p>
                                </div>
                                <button className="bg-teal-600 text-white text-sm px-4 py-2 rounded hover:bg-teal-700 transition-all">
                                    Select
                                </button>
                            </div>
                        ))}
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-lg shadow overflow-hidden border border-gray-100">
                    <div className="p-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                        <div>
                            <h3 className="text-gray-900 font-semibold text-lg">{selectedClass.name}</h3>
                            <p className="text-gray-500 text-sm">{selectedClass.time} | Section {selectedClass.section}</p>
                        </div>
                    ...
                    <div className="p-4">
                        <div className="flex gap-2 mb-4">
                            ...
                            <button onClick={() => setSelectedClass(null)} className="text-xs bg-gray-100 text-gray-600 px-3 py-1.5 rounded-lg hover:bg-gray-200 transition-colors ml-auto">
                                ← Back to Classes
                            </button>
                        </div>
                            ...
                            {students.map((s) => {
                                ...
                                return (
                                    <button key={s.rollNo} onClick={() => toggleAttendance(s.rollNo)}
                                        className={`bg-white p-3 text-center rounded-lg border-2 transition-all hover:shadow-md ${status === 'present' ? 'border-teal-500 bg-teal-50'
                        ...
                        <div className="mt-4 flex gap-3">
                            <button onClick={handleSubmit} disabled={submitted}
                                ...
                                {submitted ? <><CheckCircle2 className="w-4 h-4" /> Submitted!</> : 'Submit Attendance'}
                            </button>
                        </div>
                    </div>
                </motion.div>

    ...
    return (
        <>
            <button
                ...
                className={`fixed bottom-6 right-6 z-50 w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-600 to-violet-600 shadow-lg flex items-center justify-center transition-all duration-300 hover:scale-110 ${isOpen ? 'rotate-0' : 'animate-bounce'}`}>
                {isOpen ? <X className="w-6 h-6 text-white" /> : <MessageCircle className="w-6 h-6 text-white" />}
            </button>

            {isOpen && (
                <div className="fixed bottom-24 right-6 z-50 w-80 sm:w-96 bg-slate-800 border border-slate-600 rounded-2xl shadow-2xl overflow-hidden animate-slide-up flex flex-col" style={{ height: '480px' }}>
                    <div className="bg-gradient-to-r from-blue-600 to-violet-600 p-4 flex items-center gap-3 shadow-lg">
                        ...
                        <div>
                            <h4 className="text-white font-bold font-heading text-base">DeptBot</h4>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-900/50">
                        {messages.map((msg, i) => (
                            <div key={i} className={`flex ${msg.from === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[85%] px-4 py-3 rounded-2xl text-sm leading-relaxed shadow-md ${msg.from === 'user'
                                        ? 'bg-gradient-to-r from-blue-600 to-violet-600 text-white rounded-br-sm border border-violet-500/30'
                        ...
                        {showSuggestions && (
                            <div className="space-y-2 pt-2">
                                <p className="text-slate-300 text-xs font-medium">💡 Quick questions:</p>
                                {faqs.slice(0, 4).map((f) => (
                                    <button key={f.q} onClick={() => handleSend(f.q)}
                                        className="block w-full text-left text-xs bg-slate-700/50 hover:bg-blue-600/30 border border-slate-500/50 rounded-xl px-3 py-2.5 text-white transition-all hover:border-blue-500/50 hover:shadow-lg hover:shadow-blue-500/10">
                    ...
                    <div className="p-3 border-t border-slate-600 bg-slate-800/50 flex gap-2">
                        <input
                            type="text"
                            value={input}
                            ...
                            className="flex-1 bg-slate-700/50 border border-slate-500 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-blue-500 transition-all"
                        />
                        <button onClick={() => handleSend()}
                            className="w-10 h-10 bg-gradient-to-br from-blue-600 to-violet-600 rounded-xl flex items-center justify-center hover:opacity-90 hover:shadow-lg hover:shadow-blue-500/30 transition-all">
